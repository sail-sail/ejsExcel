                                                        ...
                                                        for ((m_c_i = t = 0, len8 = ref7.length); t < len8; m_c_i = ++ t) {
                                                            ...
                                                            if (charToNum(cItem.r.replace(/\d+/, "")) >= charToNum(ref.replace(/\d+/, "")) && Number(cItem.r.replace(/\D+/, "")) >= Number(ref.replace(/\D+/, ""))) {
                                                                if (cItem.v != null) {
                                                                    ...
                                                                    cItem.v["$t"] += "<% _hyperlinkArr_.push({ref:_charPlus_('" + ref.replace(/\d+/, "") + "',_c)+(" + Number(ref.replace(/\D+/, "")) + "+_r),location:'" + location.replace(/'/gm, "\\'") + "'}) %>";
                                                                } else {
                                                                    if (! cItem["$t"]) {
                                                                        cItem["$t"] = "";
                                                                    }
                                                                    cItem["$t"] += "<% _hyperlinkArr_.push({ref:_charPlus_('" + ref.replace(/\d+/, "") + "',_c)+(" + Number(ref.replace(/\D+/, "")) + "+_r),location:'" + location.replace(/'/gm, "\\'") + "'}) %>";
                                                                }
                                                                mciNumArr.push(m_c_i);
                                                            }
                                                        }
