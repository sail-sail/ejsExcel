 ...
 * 解析 sheet ，生成一段代码，将来用作函数体(所接受的函数参数名为`_args`，代表传入的Data)：
 * 
 * ```javascript
 * function(_args){
 * ///// begin 
 *     var buf = []; 
 *     {{beforeBuf}} //reference _data_,_ps_,etc.
 *     buf.push(Buffer.from('{{CODE PARSED}}'));
...
var parse = exports.parse = function (str, options) {
    ...
    var open = options.open || exports.open || '<%'
        ...
        , buf = [];  // array of code snippets ( Note that the element's type of `buf[]`is String)
    buf.push('var buf = [];');  // the snippets's buf here means array of Buffer instead of String
    buf.push(beforeBuf);
        ...
        if (str.slice(i, open.length + i) == open) {
            ...
            strTmp = strTmp.replace(/<c r="[A-Z]+[0-9]+"/gm, function (s) {
                ...
                if (mthArr.length === repNum) {
                     ...
                     * 计算_rc的值,
                     * ```
                     * _col=_charPlus_('${cellRn}',_c);
                     * _rc=_col+_row;
                     * ```
                     * 然后得到所追加到buf中的字符串，格式为：
                     * ```javascript
                     * <c r="'{{_rc}}'"; 
                     * ```
                     */
